...
def game_selections(input_req, wrong_input, *choices):
	...
	while True:
		...
		# Repeat the loop if invalid, to get a valid selection
			...
			print(colored(f'\nYou entered {game_sel}. ', 'light_grey', attrs=['bold']) + colored(wrong_input, 'light_cyan'))
			continue
...
Do you 1) Knock on the door, or 2) Barge right in?\n''', 'light_yellow'))
	...
	if second_sel == 'Barge':
		...
		if 1 <= second_sub_die <= 4:
			# Unlucky outcome
			print(colored('You barge in and the old crone inside sends a blast of magic your way.', 'light_red', attrs=['bold']))
			# Pause before continuing the game
			time.sleep(3)
